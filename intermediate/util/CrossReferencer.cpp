...
void CrossReferencer::print(const SymtabStack *symtabStack) const
{
    ...
    SymtabEntry *purrgramId = symtabStack->getPurrgramId();
    printRoutine(purrgramId);
}

void CrossReferencer::printRoutine(SymtabEntry *callId) const
{
    Kind kind = callId->getKind();
    cout << endl << "*** " << KIND_STRINGS[(int) kind]
         << " " << callId->getName() << " ***" << endl;
    printColumnHeadings();

    // Print the entries in the routine's symbol table.
    Symtab *symtab = callId->getRoutineSymtab();
    ...
    vector<SymtabEntry *> *callIds = callId->getSubroutines();
    if (callIds != nullptr)
    {
        for (SymtabEntry *callId : *callIds) printRoutine(callId);
    }
}
